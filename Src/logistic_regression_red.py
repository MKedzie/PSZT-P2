...
reg = LogisticRegression(random_state=42, max_iter=2000, solver='saga')
reg.fit(red_data_training, red_quality_training)
y_pred = reg.predict(red_data_test)
time_after=time.time()
print("Regression coefficient is ", reg.coef_)
#classification report
print(metrics.classification_report(red_quality_test,y_pred))
Conf_Mat = metrics.confusion_matrix(red_quality_test, y_pred)
print("The confusion matrix is\n", Conf_Mat)
#print("Accuracy is ", metrics.accuracy_score(red_quality_test, y_pred))
